  ...
  return (
    <main className='flex flex-col max-lg:max-w-xs mx-auto bg-crust p-8 pb-2 rounded-lg md:max-w-lg md:p-10 md:pb-4'>
      <div className='bg-surface0 p-4 py-8 mb-8 text-center rounded-lg'>
        <span className={`font-mono font-medium ${display.length > 5 ? 'text-4xl xl:text-6xl' : 'text-6xl'}`}>{display}</span>
      </div>
      <div className='grid grid-rows-3 gap-4 place-items-center mb-6 max-lg:place-content-center md:gap-6'>
        <input
          ...
          disabled={false}
          value={timeInput}
          className={`form-input font-mono font-medium text-lg text-center mt-0 block w-full md:mx-auto px-2 py-3 border-0 border-blue focus:ring-0 ${validTime ? 'focus:bg-surface1 bg-surface0' : 'focus:bg-red bg-red text-crust caret-peach'} rounded-lg caret-lavender transition-all ease-in-out duration-200 md:w-10/12`}
          autoFocus
        />
